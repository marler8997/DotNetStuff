...
namespace Marler.NetworkTools
{
    public class ProxyClientHandler
    {
        ...
        readonly ApplicationLayerSelectHandler serverHandler;
        ...
        public ProxyClientHandler(Socket clientSocket, ApplicationLayerSelectHandler serverHandler)
        {
        ...
        ServerInstruction HandleHeaders(String headers)
        {
            if (headers.StartsWith("CONNECT"))
            {
                ...
                //
                if (ApplicationLayerProxyLogger.logger != null)
                    ApplicationLayerProxyLogger.logger.WriteLine("[{0}] CONNECT {1}:{2}", clientEndPointString, serverString, serverPort);

                //
                // Create and connect the socket
                //
                IPEndPoint serverIPEndPoint = new IPEndPoint(DnsEndPoint.ParseIPOrResolveHost(serverString), serverPort);

                serverSocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                if (HttpSettings.proxySelector == null)
                {
                    serverSocket.Connect(serverIPEndPoint);
                }
                else
                {
                    HttpSettings.proxySelector.Connect(serverSocket, serverIPEndPoint);
                }

                //
                // Send positive connect response
                //
                clientSocket.Send(HttpSettings.ConnectionEstablishedAsBytes);
            }
            else
            {
                //
                ...
                if (hostIndex < 0)
                {
                    if (ApplicationLayerProxyLogger.errorLogger != null)
                        ApplicationLayerProxyLogger.errorLogger.WriteLine("[{0}] Missing Host header '{1}'", clientEndPointString, headers);
                    return ServerInstruction.CloseClient;
                }
                ...
                if (lastColonIndex >= 0)
                {
                    ...
                    {
                        if (ApplicationLayerProxyLogger.errorLogger != null)
                            ApplicationLayerProxyLogger.errorLogger.WriteLine(
                ...
                //	
                if (ApplicationLayerProxyLogger.logger != null)
                    ApplicationLayerProxyLogger.logger.WriteLine("[{0}] Proxy to '{1}' on port {2}", clientEndPointString, serverString, serverPort);
            ...
            if (ApplicationLayerProxyLogger.logger != null)
                ApplicationLayerProxyLogger.logger.WriteLine("[{0}] Adding Server Socket '{1}'", clientEndPointString, serverSocket.RemoteEndPoint);
        ...
        private String ParseConnectLine(String connectLine, out Int32 forwardPort)
        {
            ...
            //
            String uri = connectLine.Substring(HttpSettings.ConnectAsBytes.Length + 1); // REMOVE 'CONNECT '

            int uriEndIndex = uri.IndexOf(' ', HttpSettings.ConnectAsBytes.Length + 1);
            if (uriEndIndex <= 0)
            {
                if (ApplicationLayerProxyLogger.errorLogger != null)
                    ApplicationLayerProxyLogger.errorLogger.WriteLine(
