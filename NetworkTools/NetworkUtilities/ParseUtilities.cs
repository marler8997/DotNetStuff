...
namespace Marler.NetworkTools
{
    ...
    public static class ParseUtilities
    {
        ...
        public static ITunnel[] ParseTunnels(List<String> tunnelStrings)
        {
            ITunnel[] tunnelArray = new ITunnel[tunnelStrings.Count];
            Int32 index = 0;
        ...
        public static ITunnel ParseTunnel(String tunnel)
        {
        ...
        public static SocketConnectorFromIPAddress ParseIPAndPort(String ipAndPort)
        {
            ...
            if (colonIndex == 0) throw new ParseException(String.Format("'{0}' needs a host before the colon at the beginning", ipAndPort));
            if (colonIndex >= ipAndPort.Length - 1) throw new ParseException(String.Format("'{0}' needs a port after the colon", ipAndPort));

            String ipString = ipAndPort.Substring(0, colonIndex);
            String portString = ipAndPort.Substring(colonIndex + 1);
        ...
        public static IDirectSocketConnector ParseDirectHost(String directHost)
        {
            Int32 colonIndex = directHost.IndexOf(':');
            if (colonIndex < 0) throw new ParseException(String.Format("'{0}' needs a port, i.e. '{0}:80' would work", directHost));
            if (colonIndex == 0) throw new ParseException(String.Format("'{0}' needs a host before the colon at the beginning", directHost));
            if (colonIndex >= directHost.Length - 1) throw new ParseException(String.Format("'{0}' needs a port after the colon", directHost));

            String host = directHost.Substring(0, colonIndex);
            String portString = directHost.Substring(colonIndex + 1);
        ...
        public static ISocketConnector ParseConnectionSpecifier(this String connectionSpecifier)
        {
            ...
            Int32 percentIndex = connectionSpecifier.IndexOf('%');
            if (percentIndex >= 0)
            {
                if (percentIndex < 1)
                {
                    throw new ParseException("Expected a proxy server/port before the '%' sign, but got nothing");
                }
                if (percentIndex >= connectionSpecifier.Length - 1)
                {
                    throw new ParseException("Expected a host/port after the '%' sign, but got nothing");
                }

                String proxySpecifier = connectionSpecifier.Remove(percentIndex);
                String hostSpecifier = connectionSpecifier.Substring(percentIndex + 1);

                IDirectSocketConnector proxyConnector = ParseDirectHost(proxySpecifier);

                host = ParseHostAndPort(hostSpecifier, out hostPort);

                if (IPAddress.TryParse(host, out hostIP))
                {
                    // Use Proxy4 protocol
                    return new SocketConnectorFromProxy4(proxyConnector, null, hostIP, hostPort);
                }
                return new SocketConnectorFromProxy4a(proxyConnector, null, host, hostPort);
            }

            host = ParseHostAndPort(connectionSpecifier, out hostPort);
