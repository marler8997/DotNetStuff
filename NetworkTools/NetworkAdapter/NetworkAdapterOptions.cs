...
namespace Marler.NetworkTools
{
    ...
    public class NetworkAdapterOptions : Options
    {
        public readonly OptionStringArg communicationLogFile;
        public readonly OptionStringArg specialPortList;
        public readonly OptionIntArg readBufferSize;
        ...
        public NetworkAdapterOptions()
            : base()
        {
            communicationLogFile = new OptionStringArg('l', "CommunicationLogFile");

            specialPortList = new OptionStringArg('c', "Connect Request Port List", "Special list of ports that when connected, must establish a connection request. A connection request must be established with incoming connections if the connection is to another instance of NetworkAdapter where it's opposite end is a client that is waiting for a connection request.");

            readBufferSize = new OptionIntArg('b', "Read Buffer Size", "The size of the buffer used to hold the bytes being read from a socket");
            readBufferSize.SetDefault(4096);

            socketBackLog = new OptionIntArg('s', "Server Socket BackLog", "The maximum length of the pending connections queue");
            socketBackLog.SetDefault(32);

            AddOption(communicationLogFile);
            AddOption(specialPortList);
            AddOption(readBufferSize);
        ...
        public override void PrintHeader()
        {
            ...
            {
                Console.WriteLine("NetworkAdapter client-server [options] <client-side-host> <client-wait-mode> <port-list>");
            }
            if(!adapterTypeModeIsSet || (adapterTypeMode == AdapterType.ClientClient))
            {
                Console.WriteLine("NetworkAdapter client-client [options] <not-implemented-yet>");
            }

            if (!adapterTypeModeIsSet || (adapterTypeMode == AdapterType.ClientServer))
            {
                Console.WriteLine();
                Console.WriteLine("<client-wait-mode> = 'connect'/'c'|'immediate'/'i'|'wait'/'w'");
                Console.WriteLine("                     connect  : open client-side connection when server receives a connection");
                Console.WriteLine("                     immediate: open client-side connection immediately");
                Console.WriteLine("                     wait     : open client-side connection after server side received a 'connect request'");
            }

            if(!adapterTypeModeIsSet || (adapterTypeMode == AdapterType.ServerServer))
            {
                Console.WriteLine();
                Console.WriteLine("NetworkAdapter server-server [options] <tunnel-list>");
                Console.WriteLine();
                Console.WriteLine();
