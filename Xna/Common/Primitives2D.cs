...
namespace Marler.Xna.Common
{
    ...
    public static class Primitives2D
    {
        ...
        static GraphicsDevice graphicsDeviceForBasicEffect = null;
        static BasicEffect primitiveEffect = null;
        static VertexPositionColor[] vertices = new VertexPositionColor[5];
        ...
        public static void FillQuad(this GraphicsDevice graphics, Vector2 a, Vector2 b, Vector2 c, Vector2 d, Matrix transform, Color color)
        {
            if (graphics != graphicsDeviceForBasicEffect)
            {
                graphicsDeviceForBasicEffect = graphics;
                primitiveEffect = new BasicEffect(graphicsDeviceForBasicEffect);
                //primitiveEffect.View = Matrix.CreateLookAt(new Vector3(0, 0, ), new Vector3(0, 0, 0), new Vector3(0, 1, 0));
                //primitiveEffect.Projection = Matrix.Identity;
                //primitiveEffect.World = Matrix.Identity;
                primitiveEffect.VertexColorEnabled = true;
            }


            primitiveEffect.World = transform;

            vertices[0] = new VertexPositionColor(new Vector3(a.X, a.Y, 0), Color.Red);
            vertices[1] = new VertexPositionColor(new Vector3(b.X, b.Y, 0), Color.Green);
            vertices[2] = new VertexPositionColor(new Vector3(c.X, c.Y, 0), Color.Blue);
            vertices[3] = new VertexPositionColor(new Vector3(d.X, d.Y, 0), Color.Yellow);
            vertices[4] = vertices[0];

            for (int i = 0; i < primitiveEffect.CurrentTechnique.Passes.Count; i++)
            {
                primitiveEffect.CurrentTechnique.Passes[i].Apply();
                graphics.DrawUserPrimitives(PrimitiveType.TriangleStrip, vertices, 0, 3);
            }
        }
