...
namespace More.Pdl
{
    public class CSharpCodeGenerator : ICodeGenerator
    {
        ...
        /*
        String ArrayTypeToReflectorString(PdlArrayType arrayType)
        {
            if (arrayType == null) return "";
            if (arrayType.type == PdlArraySizeTypeEnum.BasedOnCommand) throw new NotImplementedException();
        ...
        public void GenerateReflectorConstructor(TextWriter writer, UInt32 tabs, String className, String fieldName, IntegerTypeReference type)
        {
            if (type.arrayType == null)
            {
                String prefix = (type.type == PdlType.Byte) ? "" : "BigEndian";
                writer.WriteLine(tabs * 4, "new {0}{1}Reflector(typeof({2}), \"{3}\"),",
                    prefix, type.type, className, fieldName);
            }
            else
            {
                if (type.type == PdlType.Byte)
                {
                    if (type.arrayType.type == PdlArraySizeTypeEnum.Fixed)
                    {
        ...
        {
            writer.WriteLine(tabs * 4, "new PdlFieldSerializerReflector(typeof({0}), \"{1}\", {2}),",
                className, fieldName, type.arrayType.GetArraySizeByteCount());
        }
        */
    }
